...
function encryptTravisProData (options, callback) {
    ...
    travis.authenticate({
        ...
        travis.repos(options.owner, options.repo).key.get(function (keyError, res) {
            if (!keyError) {
                return encryptData(options.data, res.key, callback);
            }

            travis.repos(options.owner, options.repo).get(function (reposError) {
...
function encryptTravisData (options, callback) {
    ...
    travis.repos(options.owner, options.repo).key.get(function (keyError, res) {
        ...
        return travis.repos(options.owner, options.repo).get(function (reposError) {
            callback(reposError ? getRepoNotFoundError(options) : keyError);
        });
    });
};
...
function encrypt (options, callback) {
    if (typeof options.slug !== 'string') {
        return callback(new TypeError('`slug` must be a string'));
    } else if (!options.slug.match(/.\/./)) {
        return callback(new Error('`slug` must be in `owner/repo` form'));
    }
